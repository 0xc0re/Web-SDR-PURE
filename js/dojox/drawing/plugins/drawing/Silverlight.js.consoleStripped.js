...
define("dojox/drawing/plugins/drawing/Silverlight", ["dojo","dijit","dojox"], function(dojo,dijit,dojox){
	...
	function(options){
		...
		this.mouse._down = function(evt){
			...
			var obj = {x:x,y:y, id:id};
			 0 && console.log(" > > > id:", id, "drawingType:", this.drawingType, "evt:", evt)
			...
			if(this._lastClickTime){
				if(this._clickTime-this._lastClickTime<this.doublClickSpeed){
					...
					// 0 && console.log("    slow:", this._clickTime-this._lastClickTime)
				}
			}
		...
		this.mouse._getXY =  function(evt){
			...
			// 0 && console.log("EVT", evt.pageX)
			for(var nm in evt){
				// 0 && console.log("..."+nm+"="+evt[nm]);
			}
			 0 && console.log("EVTX", evt.x)
			if(evt.x !== undefined){
				return {
